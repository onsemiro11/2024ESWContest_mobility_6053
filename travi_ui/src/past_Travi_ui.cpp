...
public:
    TraviUINode() : Node("travi_ui_node") {
        ...
        traffic_light_subscription_ = this->create_subscription<std_msgs::msg::String>(
            "/yolo_traffic_light", 10, std::bind(&TraviUINode::yoloTrafficLightCallback, this, std::placeholders::_1));

        // 차량 정보 구독 (YOLO 형식: x1, y1, x2, y2, depth)
        car_subscription_ = this->create_subscription<std_msgs::msg::String>(
            "/yolo_car", 10, std::bind(&TraviUINode::yoloCarCallback, this, std::placeholders::_1));

        // 차선 정보 구독 (도로 타입)
        road_type_subscription_ = this->create_subscription<std_msgs::msg::String>(
            "/road_type", 10, std::bind(&TraviUINode::roadTypeCallback, this, std::placeholders::_1));
...
private:
    ...
    // void checkAndPlayDepartureAlarm() {
    // // 신호 상태를 계속 주기적으로 확인
    //     if (shouldTriggerDepartureAlarm(traffic_status_)) {
    //         if (!alarm_playing_ && stop_status_) {
    //             departure_alarm_ = true;
    //             RCLCPP_INFO(this->get_logger(), "출발알람 켜.");
    //             playBeepSound();
    //         }
    //     } else {
    //         // 차량이 움직이거나 출발 알림 조건이 충족되지 않을 때 알람을 리셋
    //         if (!stop_status_ && alarm_playing_) {
    //             resetAlarmIfMoving();  
    //         }
    //     }
    // }
        ...
        if (stop_status_){
            if (departure_alarm_ && !alarm_playing_) {
                ...
                alarm_thread_ = std::thread([this]() {
                    system("mpg123 -q /home/hyjeon/travi_ws/src/travi_ui/src/beep.mp3");
    ...
    void resetAlarmIfMoving() {
        if (!stop_status_ && departure_alarm_) {
            ...
            system("pkill -f 'mpg123 -q /home/hyjeon/travi_ws/src/travi_ui/src/beep.mp3'");
    ...
    // 비디오 처리 함수
    void processVideo() {
        cv::VideoCapture cap("src/travi_ui/src/example.mp4");
        if (!cap.isOpened()) {
            RCLCPP_ERROR(this->get_logger(), "Failed to open video file");
            return;
        }

        cv::VideoWriter video("output.mp4", cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), 30, cv::Size(frame_width, frame_height));
        ...
        while (rclcpp::ok() && cap.read(frame)) {
            ...
            // 차선 폴리곤 그리기
            drawTransparentPolygon(frame, lanePts_, cv::Scalar(255, 200, 200, 128), 0.5);

            // 도로 타입에 따른 화살표 그리기
            cv::Point arrowCenter(frame_width / 2, frame_height - 100);
            drawCustomArrow(frame, arrowCenter, road_type_, cv::Scalar(255, 255, 255), 0.7);

            // 신호등 UI 그리기
            drawTrafficLightUI(frame, stop_status_, traffic_status_);
    ...
    void drawCustomArrow(cv::Mat &img, cv::Point center, const std::string &direction, cv::Scalar color, double alpha = 0.7, int arrow_width = 30, int arrow_height = 50, int arrow_head = 40) {
        ...
        if (direction == "RoadMarkArrow_Straight") {
            arrow_body = {
    ...
    void drawTrafficLightUI(cv::Mat &img, bool isActive, const std::string &lightState) {
        ...
        if (greenLeftArrow == cv::Scalar(0, 255, 0)) {
            drawCustomArrow(img, cv::Point(top_left.x + 150, top_left.y + height / 2), "left", greenLeftArrow, 0.7, 10, 20, 10);
        }
    }
};
