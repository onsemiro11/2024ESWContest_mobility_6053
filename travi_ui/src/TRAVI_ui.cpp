...
class TraviUINode : public rclcpp::Node {
    ...
    // map variable to store sizes of the bounding boxes as int
    std::unordered_map<int, int> current_sizes_;
    std::unordered_map<int, int> previous_sizes_;
    ...
    // 박스 표시 유지 시간 (초)
    const double BOX_DISPLAY_DURATION = 0.08;
...
public:
    TraviUINode() : Node("travi_ui_node") {
        ...
        frame_subscription_ = this->create_subscription<sensor_msgs::msg::Image>(
            "/camera/color/image_raw2", 10, std::bind(&TraviUINode::image_callback, this, std::placeholders::_1));
        ...
        // 출발 알림 이미지 로드
        departure_image_traffic_ = loadAndResizeImage("move_car.png", 200, 180);
        departure_image_car_ = loadAndResizeImage("move_frontcar.png", 200, 180);
...
private:
    ...
    void checkAndPlayDepartureAlarm() {
        ...
        } else if (departure_alarm_ && !alarm_playing_ && (stop_status_ || front_car_)) {
    ...
    void playBeepSound() {
        if (stop_status_ || front_car_){
            if (departure_alarm_ && !alarm_playing_) {
                alarm_playing_ = true;

                // 알람 소리 재생을 비동기로 처리
                alarm_thread_ = std::thread([this]() {
                    system("mpg123 -q /home/travi/check_ws/src/travi_ui/src/beep.mp3");
    ...
    void resetAlarmIfMoving() {
        if (!stop_status_ && departure_alarm_) {
            ...
            system("pkill -f 'mpg123 -q /home/travi/check_ws/src/travi_ui/src/beep.mp3'");
    ...
    void drawBBox(cv::Mat &frame, const BBox &bbox) {
        ...
        // RCLCPP_INFO(this->get_logger(), "Drawing coordinates: [%f, %f, %f, %f]",  bbox.x1,  bbox.y1,  bbox.x2,  bbox.y2);
        // 박스 그리기
    ...
    void roadTypeCallback(const std_msgs::msg::String::SharedPtr msg) {
        ...
        // RCLCPP_INFO(this->get_logger(), "Road type data received: %s", msg->data.c_str());
    ...
    void yoloTrafficLightCallback(const std_msgs::msg::String::SharedPtr msg) {
        ...
        // RCLCPP_INFO(this->get_logger(), "1Converted coordinates: [%f, %f, %f, %f], ID: %d, Class: %s", x1, y1, x2, y2, id, class_label.c_str());
        ...
        if (traffic_light_times_.find(id) == traffic_light_times_.end()) {
            ...
            current_traffic_light_label_ = "nolabel";
        } else {
        ...
        for (auto it = traffic_light_times_.begin(); it != traffic_light_times_.end(); ) {
            if (tracked_lights_.find(it->first) == tracked_lights_.end()) {  // Traffic light ID is no longer detected
                ...
                if (time_since_last_seen.seconds() >= 6) {  // If 5 seconds have passed since stop_status_ was set to true
                    stop_status_ = false;
    ...
    void yoloCarCallback(const std_msgs::msg::String::SharedPtr msg) {
        ...
        while (std::getline(ss, box_data, ';')) {
            ...
            current_sizes_[id] = std::abs(x2 - x1) * std::abs(y2 - y1);

            if (previous_sizes_.find(id) != previous_sizes_.end()) {
                int previous_size = previous_sizes_[id];
                int current_size_ = current_sizes_[id];
                // Check for movement
                if (current_size_ < previous_size * 0.95) {
                    RCLCPP_INFO(this->get_logger(), "Car is moving, prepare to start!");
                    front_car_ = true;
                    departure_alarm_ = true;
                }
                // Update the previous size for the given ID
                previous_sizes_[id] = current_sizes_[id]; 
            } else {
                // If the ID is not found, initialize it with the current size
                previous_sizes_[id] = current_sizes_[id];
            }
    ...
    void processVideo() {
        ...
        cv::namedWindow("Travi UI", cv::WINDOW_KEEPRATIO);
        ...
        while (rclcpp::ok()) {
            ...
            // front_car_ = true;
            /////////////////////////////////////
            // log current size and previous size

            // RCLCPP_INFO(this->get_logger(), "Departure alarm: %d, Front car: %d", departure_alarm_, front_car_);
            ...
            if (departure_alarm_ && alarm_playing_) {
                ...
                } else if (road_type_ == "Unprotected_LeftTurn" || road_type_ == "RoadMarkArrow_Left" || road_type_ == "RoadMarkArrow_Right") {
                    arrow = &left_arrow_;
                } else {
                    arrow = &straight_arrow_;
                }
            ...
            // RCLCPP_INFO(this->get_logger(), "Current traffic light label: %s", current_traffic_light_label_.c_str());
